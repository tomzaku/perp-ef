    ...
    <div ref={menuRef} className="fixed bottom-6 right-6 z-30 flex flex-col items-end gap-2">
      ...
      {expanded && (
        <div className="flex flex-col items-end gap-2 animate-fade-in">
          ...
          {/* Read Aloud */}
          <button
            onClick={() => openPanel('readAloud')}
            className="flex items-center gap-2.5 pl-4 pr-3 py-2.5 rounded-full shadow-lg border border-accent-purple/30 bg-bg-card hover:bg-accent-purple/10 transition-all cursor-pointer group"
          >
            <span className="text-sm font-medium text-text-primary group-hover:text-accent-purple transition-colors whitespace-nowrap">
              Read Aloud
            </span>
            <span className="w-9 h-9 rounded-full bg-accent-purple/15 text-accent-purple flex items-center justify-center shrink-0">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
